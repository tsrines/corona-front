...
class StatesContainer extends React.Component {
  ...
  displayStates = () => {
    let states = this.state.states.filter(state =>
      state.state.toLowerCase().includes(this.state.search.toLowerCase())
    );
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Table striped selectable sortable unstackable celled singleLine>
          <Table.Header>
            <Table.Row>
              <Table.HeaderCell
          ...
          <Table.Body>{this.displayStates()}</Table.Body>
        </Table>
      </div>
    );
  }
